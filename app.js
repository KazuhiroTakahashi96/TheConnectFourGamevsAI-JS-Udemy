...
// game parameters
const DELAY_AI = 0.5; //seconds for AI to take its turn
...
class Cell {
  ...
  draw(canvasContext) {
    ...
    if (this.winner || this.highlight != null) {
      ...
      // draw circle around the parameter
...
function checkWin(row, col) {
  // getting all cells from each direction
  let diagonalLeft = [],
    diagonalRight = [],
    horizontal = [],
    vertical = [];
  ...
  // if any habe four in a row, return win
  return (
...
function click() {
  ...
  // if it is the players turn, the other player should not be able to click
  if (!playersTurn) {
    return;
  }
...
function createGrid() {
  ...
  // set up cell size and margin
  let cell, marginX, marginY;

  // device poortrait orientation
